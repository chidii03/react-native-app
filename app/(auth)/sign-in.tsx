...
const SignIn = () => {
  ...
  const validate = (): boolean => {
    ...
    if (em && !emailRegex.test(em)) { setEmailErr("Enter a valid email address"); ok = false; }
    if (!password)                  { setPwErr("Password is required");          ok = false; }
    else if (password.length < 8)   { setPwErr("Password must be at least 8 characters"); ok = false; }
    else                            setPwErr("");
    return ok;
  };
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (rt) router.replace((autoPlay === "true" ? rt + "?autoPlay=true" : rt) as any);
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={S.scroll} keyboardShouldPersistTaps="always" showsVerticalScrollIndicator={false}>
          ...
          <View style={S.card}>
            ...
            {/* Password — FIX: eye button contained, never overflows on small screens */}
            <View style={S.field}>
              ...
              <View style={[S.inputRow, !!pwErr && S.inputRowErr]}>
                <View style={S.iconBox}><Ionicons name="lock-closed-outline" size={16} color={pwErr ? "#ef4444" : "#AB8BFF"} /></View>
                {/* flex:1 + minWidth:0 ensures input shrinks before the eye button gets pushed out */}
                <TextInput ref={pwRef} style={S.input} value={password}
                  ...
                  {...(Platform.OS === "web" ? { outlineStyle: "none" } as any : {})} />
                {/* Eye button: fixed 36×36 so it never gets squished on small screens */}
...
const S = StyleSheet.create({
  ...
  backBtn:    { alignSelf: "flex-start", width: 36, height: 36, borderRadius: 18, backgroundColor: "rgba(255,255,255,0.08)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", alignItems: "center", justifyContent: "center", marginBottom: 14 },
  card:       { width: "100%", maxWidth: 480, alignSelf: "center", backgroundColor: "rgba(12,4,30,0.95)", borderRadius: 22, padding: 20, borderWidth: 1, borderColor: "rgba(255,255,255,0.09)" },
  ...
  contextBox: { flexDirection: "row", alignItems: "flex-start", gap: 8, backgroundColor: "rgba(171,139,255,0.08)", borderWidth: 1, borderColor: "rgba(171,139,255,0.2)", borderRadius: 12, padding: 10, marginBottom: 16 },
  contextTxt: { color: "rgba(171,139,255,0.85)", fontSize: 12, flex: 1, lineHeight: 17 },
  errBox:     { flexDirection: "row", gap: 8, alignItems: "flex-start", backgroundColor: "rgba(239,68,68,0.08)", borderWidth: 1, borderColor: "rgba(239,68,68,0.25)", borderRadius: 12, padding: 12, marginBottom: 16 },
  ...
  // ── inputRow: flex row, overflow hidden so eye button never pokes outside ──
  ...
  iconBox:    { width: 28, height: 28, borderRadius: 7, backgroundColor: "rgba(171,139,255,0.1)", alignItems: "center", justifyContent: "center", marginRight: 6, flexShrink: 0 },
  // ── input: minWidth 0 ensures it shrinks rather than pushing eye button out ──
  input:      { flex: 1, minWidth: 0, color: "#fff", fontSize: 14, paddingVertical: Platform.OS === "web" ? 10 : 11, paddingHorizontal: 2 },
  // ── eyeBtn: fixed 36×36, never resizes, never overflows ──
  eyeBtn:     { width: 36, height: 36, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  fieldErr:   { color: "#ef4444", fontSize: 10, marginTop: 3, marginLeft: 2 },
  forgot:     { color: "#AB8BFF", fontSize: 11, fontWeight: "700" },
  btn:        { borderRadius: 12, overflow: "hidden", marginBottom: 16, marginTop: 4 },
  ...
  divRow:     { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 },
  ...
  oauthBtn:   { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, borderRadius: 999, borderWidth: 1, paddingVertical: 11, minHeight: 44, backgroundColor: "rgba(243,244,246,0.08)", borderColor: "rgba(229,231,235,0.35)" },
  oauthTxt:   { fontSize: 13, fontWeight: "800", color: "#f9fafb" },
  outBtn:     { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 11, paddingVertical: 13, borderWidth: 1.5, borderColor: "rgba(171,139,255,0.35)", marginBottom: 8 },
